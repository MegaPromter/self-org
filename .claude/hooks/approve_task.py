...
def main():
    ...
    if not табло.get("задача"):
        ...
        return 1
    сейчас = datetime.now()
    # Откуда пришёл штамп: кнопка Run внутри Claude Code или своё
    # окно терминала. Пишем в табло — чтобы всегда было видно,
    # каким путём получено согласование.
    способ = (
        "кнопка Run в Claude Code"
        if os.environ.get("CLAUDECODE")
        else "терминал пользователя"
    )
    табло["одобрено"] = True
    ...
    print(
        ...
        f"({табло['одобрено_в']}, {способ}). "
        f"Ассистент может приступать к коду."
    )
    return 0
