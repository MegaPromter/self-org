...
def канал():
    ...
    Список каналов возвращается как был: настоящий канал Telegram
    регистрируется при старте приложения, и стирать его нельзя —
    он нужен другим тестам.
    """
    принятые = []
...
def test_повтор_о_просроченном_не_чаще_недели(страховка):
    ...
    # Срок повтора считается от напоминания, которое дошло:
    # недоставленное устаревает и в счёт не идёт (заметка
    # «Telegram-бот», правило 3).
    Notification.objects.update(status=Notification.Status.SENT)
