...
class Дело:
    ...
    тон: str = ""  # оттенок карточки по разделу
    кружок: str = ""  # тот же оттенок для кружка со значком
    значок: str = ""  # картинка раздела; пусто — раздел без значка
    ...
    @property
    def срок(self):
        """Срок словами в правом углу карточки: «через 7 дн».

        Заменяет прежнюю цветную пилюлю «в норме»: состояние видно
        по цвету, а место занимает то, чего по цвету не узнать —
        сколько осталось (правило 5 заметки «Порядок на экранах»).
        """
        return _срок_словами(self.status)

    @property
    def как_повторяется(self):
        return _как_повторяется(self.obligation)
    ...
    def подпись(self, с_разделом=True):
        """Вторая строка: раздел, к чему относится, как повторяется.

        На странице раздела название раздела не повторяем — оно
        и так в заголовке. «Сколько осталось» отсюда ушло в правый
        угол карточки, а его место заняло правило повтора: без него
        непонятно, «через 7 дн» — это раз в неделю или раз в год.
        """
        части = []
...
class Плитка:
    ...
    адрес: str = ""
    активен: bool = False
    значок: str = ""  # картинка раздела в ленте и меню
    кружок: str = ""  # оттенок кружка, тот же, что у карточек дел
...
def склонение(число, одно, два, много):
    """«1 дело», «3 дела», «5 дел» — русский счёт словами."""
    if 11 <= число % 100 <= 14:
        return много
    остаток = число % 10
    if остаток == 1:
...
class Сводка:
    ...
    @property
    def слово_дел(self):
        """«дело», «дела», «дел» — по числу рядом."""
        return склонение(self.всего, "дело", "дела", "дел")
...
def _срок_словами(статус):
    """Короткий срок для правого угла карточки.

    Одно место — один ответ на вопрос «когда»: «через 7 дн»,
    «срок сегодня», «просрочено на 85 дн». Есть и время,
    и счётчик — обе части через точку: «через 7 дн · ≈400 км».
    """
    if статус.state is State.NO_DATA:
        return "нет данных"
    if статус.state is State.NO_DEADLINE:
        return "без срока"

    просрочено = (статус.days_left is not None and статус.days_left < 0) or (
        статус.meter_left is not None and статус.meter_left < 0
    )
    части = []
    if статус.days_left is not None:
        части.append(
            "срок сегодня"
            if статус.days_left == 0
            else f"{abs(статус.days_left)} дн"
        )
    if статус.meter_left is not None:
        префикс = "≈" if статус.is_estimate else ""
        части.append(
            f"{префикс}{short_number(abs(статус.meter_left))} "
            f"{статус.meter_unit}"
        )
    if not части:
        return ""
    текст = " · ".join(части)
    if просрочено:
        return f"просрочено на {текст}"
    # «Срок сегодня» уже сам себе предлог — «через» перед ним лишнее.
    return текст if части[0] == "срок сегодня" else f"через {текст}"


# Интервал длиной в единицу читается человеческими словами:
# «каждые 1 месяцев» не говорят.
РАЗ_В = {
    Obligation.IntervalUnit.DAYS: "каждый день",
    Obligation.IntervalUnit.WEEKS: "раз в неделю",
    Obligation.IntervalUnit.MONTHS: "раз в месяц",
    Obligation.IntervalUnit.YEARS: "раз в год",
}


def _как_повторяется(обязательство):
    """«каждые 6 месяцев», «каждые 8 000 км», «раз в год 14 сентября».

    Правило дела словами — чтобы не гадать по одному сроку, часто
    ли это бывает (правило 10 заметки «Порядок на экранах»).
    """
    о = обязательство
    части = []
    if о.rule_kind in (Obligation.RuleKind.TIME, Obligation.RuleKind.BOTH):
        if о.time_kind == Obligation.TimeKind.ONCE:
            части.append("один раз")
        elif о.time_kind == Obligation.TimeKind.SOMEDAY:
            части.append("когда получится")
        elif о.time_kind == Obligation.TimeKind.INTERVAL and о.interval_value:
            if о.interval_value == 1:
                части.append(РАЗ_В.get(о.interval_unit, "каждый раз"))
            else:
                части.append(
                    f"каждые {о.interval_value} "
                    f"{о.get_interval_unit_display()}"
                )
        elif о.time_kind == Obligation.TimeKind.ANNUAL and о.annual_month:
            день = datetime.date(2000, о.annual_month, о.annual_day)
            части.append(f"раз в год {date_format(день, 'j E')}")
    if (
        о.rule_kind in (Obligation.RuleKind.METER, Obligation.RuleKind.BOTH)
        and о.meter_interval
        and о.meter
    ):
        части.append(
            f"каждые {short_number(о.meter_interval)} {о.meter.unit}"
        )
    return " или ".join(части)


def _расходы(обязательство):
    """«18 500 ₽ за 3 раза» — сколько дело стоило за всю историю."""
    записи = [
        в.cost for в in обязательство.completions.all() if в.cost is not None
    ]
    if not записи:
        return "не записано"
    раз = len(записи)
    # «за 1 раз», «за 3 раза», «за 5 раз» — по-русски окончание
    # зависит от последних цифр.
    хвост = "раза" if раз % 10 in (2, 3, 4) and раз % 100 not in (
        12,
        13,
        14,
    ) else "раз"
    return f"{short_number(sum(записи))} ₽ за {раз} {хвост}"
...
# Сколько оттенков в палитре разделов (сами цвета — в base.html).
# Разделов больше палитры — оттенки идут по кругу.
ТОНОВ = 8


def _оформление():
    """Чем раздел узнаётся на экране: оттенок и значок.

    Цвет нигде не заводится руками: он следует из места раздела
    в списке, поэтому одинаков на всех экранах и не мельтешит.
    Значок, наоборот, выбирается в справочнике — но ездит вместе
    с цветом, чтобы шаблоны спрашивали об оформлении одно место
    (правило 3 заметки «Значки разделов»).
    """
    return {
        str(раздел.pk): _вид(номер % ТОНОВ + 1, раздел.icon)
        for номер, раздел in enumerate(Category.objects.all())
    }


def _вид(номер, значок):
    """Тройка «класс карточки, класс кружка, значок» для шаблонов."""
    return (f"тон{номер}", f"к{номер}", значок)
...
def _дела(user, today):
    ...
    for обязательство in запрос:
        ...
        тон, кружок, значок = оформление.get(
            _ключ(раздел), ВИД_БЕЗ_РАЗДЕЛА
        )
        дела.append(
            Дело(
...
def _плитки(дела):
    ...
    for номер, раздел in enumerate(Category.objects.all()):
        ...
        _, кружок, значок = _вид(номер % ТОНОВ + 1, раздел.icon)
        плитки.append(
            Плитка(
                str(раздел.pk),
                раздел.name,
...
def _меню(все_дела, активный):
    ...
    # Пустые разделы уходят в конец: они нужны как ориентир «куда
    # заводить», но занимать место наравне с живыми не должны
    # (правила 3 и 4 заметки «Порядок на экранах»).
    пункты += [п for п in разделы if not п.пусто]
    return пункты, [п for п in разделы if п.пусто]
...
def _адрес_нового(request, ключ=None):
    """Куда ведёт «+ дело»: одна кнопка на странице, в сводке.

    На странице раздела она подставляет этот раздел — ради чего
    раньше была вторая такая же ссылка в шапке списка.
    """
    хвост = {"назад": request.get_full_path()}
    if ключ and ключ != БЕЗ_РАЗДЕЛА:
        хвост["раздел"] = ключ
    return f"{reverse('obligation_new')}?{urlencode(хвост)}"
...
def _каркас(request, все_дела, показанные, today, активный, ключ=None):
    ...
    меню, пустые = _меню(все_дела, активный)
    return {
        "меню": меню,
        "пустые_разделы": пустые,
        ...
        "адрес_нового": _адрес_нового(request, ключ),
...
def section(request, key):
    ...
    _, кружок, значок = _оформление().get(key, ВИД_БЕЗ_РАЗДЕЛА)
...
def _страница_формы(request, форма, заголовок, назад, удаление=None):
    """Обвязка страницы с формой.

    Меню и сводки здесь нет: заполняющему форму они не нужны,
    а место занимают (правило 2 заметки «Порядок на экранах»).
    """
    return render(
        request,
        "core/form.html",
...
def obligation_detail(request, pk):
    ...
    статус = compute_status(обязательство, today)
    выполнения = list(
        обязательство.completions.select_related("done_by").order_by(
            "-date", "-id"
        )
    )
    к_чему = [
        str(ч)
        for ч in (
            обязательство.item or обязательство.person,
            обязательство.effective_category,
        )
        if ч
    ]
    _, кружок, значок = _оформление().get(
        _ключ(обязательство.effective_category), ВИД_БЕЗ_РАЗДЕЛА
    )
    контекст.update(
        {
            "дело": дело,
            ...
            "статус": статус,
            # Четыре факта в шапке карточки — правило 8 заметки
            # «Порядок на экранах».
            "срок_словами": _срок_словами(статус),
            "к_чему": " · ".join(к_чему) or "ни к чему не привязано",
            # Значок раздела рядом с фактом «к чему относится»:
            # слово там остаётся, значок стоит перед ним. Берём его
            # по разделу самого дела, а не из списка на экране:
            # закрытого разового дела в том списке нет, а карточка
            # у него открывается.
            "значок": значок,
            "кружок": кружок,
            "как_повторяется": _как_повторяется(обязательство)
            or "правило не задано",
            "последний_раз": выполнения[0] if выполнения else None,
            "расходы": _расходы(обязательство),
        }
    )
...
def catalog(request):
    ...
    return render(
        ...
        "core/catalog.html",
        {
            ...
            "разделы": _разделы_справочника(),
            "назад": request.get_full_path(),
        },
    )


def _разделы_справочника():
    """Разделы для страницы справочников — вместе с их оформлением.

    Оттенок кружка тот же, что у карточек дел раздела: пользователь
    выбирает значок и сразу видит его в том цвете, в каком встретит
    на главной.
    """
    строки = []
    for номер, раздел in enumerate(Category.objects.all()):
        _, кружок, значок = _вид(номер % ТОНОВ + 1, раздел.icon)
        строки.append({"раздел": раздел, "значок": значок, "кружок": кружок})
    return строки


def _страница_справочника(request, шаблон, данные):
    """Страница справочника: список без меню и сводки (правило 2)."""
    контекст = {"назад": request.get_full_path()}
